      ...
      >
        ...
        <div
          className={cn(
            ...
            iconSide === 'right' ? 'justify-between w-full' : ''
          )}
        >
          {iconSide === 'right' ? text && <div className={cn('min-w-0 truncate')}>{text}</div> : null}
          {DefaultIcon ? (
            <DefaultIcon
              size={iconSize}
              color={iconColor}
              fill={iconFillColor ? iconFillColor : 'currentColor'}
              strokeWidth={iconStrokeWidth}
              stroke={iconTextColor}
              className={cn(
                'block mr-0 transition-all ease-in-out duration-300 group-hover:translate-x-[-8px] h-6 w-6 ',
                HoverIcon ? 'group-hover:hidden' : ''
                // iconTextColor ? iconTextColor : 'text-gray-500'
              )}
            />
          ) : null}
          {HoverIcon ? <HoverIcon className="hidden group-hover:block h-6 w-6 text-gray-700" /> : null}
          {iconSide === 'left' && text && <div className={cn('min-w-0 truncate')}>{text}</div>}
        </div>
      </Comp>
    );
  }
