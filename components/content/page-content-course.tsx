  ...
  return (
    <div className="pt-8 pb-2 px-6 md:px-4 w-full max-w-[52rem] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-8 pb-2 px-6 md:px-4 w-full max-w-[52rem] mx-auto">
        {modulesByCourse.map((module, index) => (
          <DynamicLink
            key={module.id}
            includePathname
          ...
          >
            <Card title={`Start Learning: ${module.title}`} className="h-full min-h-[236px] md:min-h-max hover:bg-muted/50 p-0 flex flex-col shadow-sm aspect-auto overflow-hidden">
              ...
              >
                {module.thumbImg ?
                  <Image
                    ...
                    alt={`Course Module ${index + 1}: ${module.title}`}
                    fill
